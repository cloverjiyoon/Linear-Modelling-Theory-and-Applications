...
```{r globalParameters,echo=FALSE,cache=FALSE}
library('knitr')
set.seed(1234)
...
$V(\mathbf{b}) = \sigma_{\epsilon}^{2}(\X'\X)^{-1}$, show that the sampling 
...
\begin{enumerate}
  [(a)] *If we use $\hat{Y}_0 = \x_0' \b$ to estimate $E(Y_0)$, then the
  ...
  %%-------------------------------
  [(b)]
  *We may be interested not in estimating the expected value of
  ...
  Use the results in parts (a) and (b), along with the Canadian
...
```
  ...
  Suppose that the methods of this problem are used to forecast a
...
#3. Show that in the linear model, the square of the (sample)
