...
#' cross validated. If \code{params} is a data.frame, each row of arguments will
#' be cross-validated.
...
#' at once (see \code{\link[=iterators:iter]{iter}}).
#' @param .doSEQ logical flag indicating whether cross validation should be run
#' sequentially or with \code{\%dopar\%}. 
#' 
#' @details This function leverages the \code{\link{foreach}} and 
#' \code{\link[=iterators:iter]{iter}} to perform \code{k}-fold cross validation in a distributed
...
#' \code{c(arg1, ..., argN)} may be very quick. Hence, the time it takes to send
#' off \code{proc}, the \code{data} and the appropriate combinations of
#' \code{params} may overwhelm the actual computation time. In this
#' instance, one should consider changing \code{.chunkSize} from 1 to \code{n}
...
kFoldCV <- function(proc, k, data, params, .rngSeed = 1234,
  ...
  require(foreach)
  require(iterators)
  # randomly partition data -- this will throw a warning if nrow(data) %% k != 0
  set.seed(.rngSeed)
